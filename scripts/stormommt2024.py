...
Original file is located at
    https://colab.research.google.com/drive/16-gyLvZcyKG5Q3QjDXDOXh0yxSpgM7-8
"""
...
Dati = {
    ...
    'tempo_max': 1000,  # Numero di istanti temporali simulati
    ...
    'x_centro_ostacolo': [80*x for x in [2, 8, 5, 3, 15, 15, 17.97, 9.3293, 6.7429, 13.7446, 3.9999, 7.5]],       # Coordinata x centro ostacolo
    'y_centro_ostacolo': [80*y for y in [3, 1, 0, 10, -0.4, 10.4, 8.8592, 1.8270, 8.3421, 5.8782, 6.3879, 5]],  # Coordinata y centro ostacolo
    'larghezza_ostacolo': [80*l for l in [1.0, 0.3, 0.2, 0.2, 0.8, 0.7, 0.4524, 0.7069, 0.7070, 0.9748, 0.869, 0.6]],  # Larghezza dell'ostacolo
    'legame_lr':  0.0, #0.1, # Probabilità del legame lungo raggio
    'r': 10, # Raggio legame di vicinanza
    'seed_posizione_iniziale': 1, # Seed per la generazione delle posizioni iniziali
    'seed_matrice_iniziale': 1, # Seed per la generazione dell'adiacenza iniziale
    'seed_aggiornamenti': 1, # Seed per la generazione della parte casuale dell'aggiornamento della direzione
    'vis_grafo': False # Visualizza il grafo in tempo reale
}
...
def PlotConfiguration(fig, Dati, x, y, u, v, kk, leaders):
    ...
    if Dati["Ostacolo"].startswith("Cerchio"):
      for x_center, y_center, radius in zip(Dati["x_centro_ostacolo"],
                                          Dati["y_centro_ostacolo"],
                                          Dati["larghezza_ostacolo"]):
    ...
    # if RunningInCOLAB:
    #   # cv2_imshow('Image', cv2.cvtColor(img_plot, cv2.COLOR_RGBA2BGR))
    #   cv2_imshow(img_plot)
    # else:
    #   graph_image = np.array(fig.canvas.get_renderer()._renderer)
    #   # image = cv2.cvtColor(img_plot, cv2.COLOR_RGBA2BGR)
...
class PercorsoLeader:
  def __init__(self, Dati):
    # dati scelti arbitrariamente: farsi guidare
    self.kk = 0
    ...
    # self.theta = np.random.rand(Dati["n_leaders"], 1) * 0.2 - 0.1
  ...
  def newtheta(self):
    return self.theta # + 0.001 * 2*np.pi

def StochasticMatrixUndirected(n, p, seed=None):
  """
...
def SimulaStormo(Dati):
  ...
  if Dati["Ostacolo"].startswith("Cerchio"):
    for ii in range(n):
      for jj in range(len(Dati["x_centro_ostacolo"])):
        if (x[ii] - Dati["x_centro_ostacolo"][jj])**2 + \
            (y[ii] - Dati["y_centro_ostacolo"][jj])**2 <= \
            Dati["larghezza_ostacolo"][jj]**2:
          x[ii] = 0
  ...
  A1, _, _ = StochasticMatrixUndirected(n,1-Dati["legame_lr"], Dati["seed_matrice_iniziale"])
  ...
  for kk in range(Dati["tempo_max"]):
    ...
    #   pos = {i: (x[i][0], y[i][0]) for i in range(n)}
    #   ax = nx.draw(G, pos, with_labels=False, node_color=np.sum(A, axis=1), edge_color='k', node_size=50, cmap='jet')
    #   # plt.colorbar(ax=ax)
    #   if Dati["Ostacolo"].startswith("Cerchio"):
    #     Circle(Dati["x_centro_ostacolo"], Dati["y_centro_ostacolo"], Dati["larghezza_ostacolo"])
    #   plt.show()
    ...
    if Dati["Ostacolo"].startswith("Cerchio"):
        for jj in range(len(Dati["x_centro_ostacolo"])):
            if (x[ii] - Dati["x_centro_ostacolo"][jj])**2 + (y[ii] - Dati["y_centro_ostacolo"][jj])**2 < Dati["larghezza_ostacolo"][jj]**2:
    ...
    if Dati["Ostacolo"].startswith("Cerchio"):
        for jj in range(len(Dati["x_centro_ostacolo"])):
            for ii in range(n):
                if ((x[ii] - Dati["x_centro_ostacolo"][jj])**2 + (y[ii] - Dati["y_centro_ostacolo"][jj])**2) < (Dati["larghezza_ostacolo"][jj] + 0.25)**2 and (x[ii] - Dati["x_centro_ostacolo"][jj] < 0):
                    theta[ii] = np.arctan((y[ii] - Dati["y_centro_ostacolo"][jj]) / (x[ii] - Dati["x_centro_ostacolo"][jj]))
